...
function About() {
  ...
  const filteredPlayers = [...players].sort((a, b) => filter === "id" ? a[filter] - b[filter] : b[filter] - a[filter]).slice(0, search.amount)
  const filteredByTextPlayers = filteredPlayers.filter(
    (player) =>
      player.surname.toLowerCase().indexOf(search.text.toLowerCase().toString()) !== -1
  )

  return (
    <div className="stat-hub">
      
      { isPlayersLoading ? <MyLoader /> :
      <div>
      ...
      <div>
        <MySelect
        value={search.amount}
        onChange={(sort) => setSearch({ ...search, amount: Number(sort) })}
      ...
      <table>
        <thead>
          <tr>
            ...
            <th onClick={() => setFilter("yellow_cards")}>Yellow cards</th>
            <th onClick={() => setFilter("red_cards")}>Red cards</th>
            <th onClick={() => setFilter("average_rating")}>Rating</th>
          </tr>
        </thead>
        <tbody>
          {filteredByTextPlayers.map((player, index) => <tr><td>{index + 1} .</td><td>{player.name}</td><td>{player.surname}</td><td>{player.goals}</td><td>{player.assists}</td><td>{player.yellow_cards}</td><td>{player.red_cards}</td><td>{player.average_rating}</td></tr> )}
        </tbody>
      </table>
      </div>
      </div>
